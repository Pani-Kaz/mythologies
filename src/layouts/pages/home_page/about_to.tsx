export function HomepageAboutTo() {
  return (
    <section className="py-10 flex flex-col gap-4 w-full justify-center items-center text-center max-lg:text-start max-lg:justify-start max-lg:items-start">
      <h2 className="text-2xl font-bold max-w-[60%] max-lg:max-w-full">
        O que são as Mitologias?
      </h2>
      <p className="text-xs font-medium max-w-[60%] max-lg:max-w-full">
        Mitologias são conjuntos de histórias e crenças que explicam a origem do
        mundo, fenômenos naturais e valores culturais por meio de narrativas
        simbólicas. Esses mitos, geralmente envolvendo deuses, heróis e forças
        sobrenaturais, ajudam a entender mistérios da existência e refletem os
        valores de cada sociedade. Presentes em culturas como a grega, nórdica e
        hindu, as mitologias transmitem sabedoria, estruturam tradições e
        inspiram a arte e a literatura, conectando o passado ao presente.
      </p>
    </section>
  );
}
